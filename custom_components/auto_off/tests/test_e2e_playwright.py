...
class TestAutoOffIntegrationSetup:
    ...
    async def test_create_group_via_service(self, ha_with_integration, reset_test_entities):
        ...
        await ha_with_integration.create_auto_off_group(
            group_name=group_name, sensors=["binary_sensor.test_motion"], targets=["light.test_light"], delay=5
        )
        ...
        # Cleanup
        await ha_with_integration.call_service("auto_off", "delete_group", {"group_name": group_name})
...
class TestAutoOffFunctionality:
    ...
    async def test_light_stays_on_with_motion(self, ha_with_integration, reset_test_entities):
        ...
        try:
            ...
            await ha_with_integration.call_service(
                "input_boolean", "turn_on", {"entity_id": "input_boolean.test_motion_state"}
            )
            await asyncio.sleep(1)

            # Turn on light
            await ha_with_integration.call_service(
                "input_boolean", "turn_on", {"entity_id": "input_boolean.test_light_state"}
            )
            await asyncio.sleep(1)
    ...
    async def test_light_turns_off_after_motion_stops(self, ha_with_integration, reset_test_entities):
        ...
        try:
            ...
            await ha_with_integration.call_service(
                "input_boolean", "turn_off", {"entity_id": "input_boolean.test_motion_state"}
            )
    ...
    async def test_multiple_sensors(self, ha_with_integration, reset_test_entities):
        ...
        try:
            ...
            await ha_with_integration.call_service(
                "input_boolean", "turn_on", {"entity_id": "input_boolean.test_motion_2_state"}
            )
    ...
    async def test_multiple_targets(self, ha_with_integration, reset_test_entities):
        ...
        try:
            ...
            await ha_with_integration.call_service(
                "input_boolean", "turn_on", {"entity_id": "input_boolean.test_light_2_state"}
            )
            await asyncio.sleep(1)
    ...
    async def test_switch_target(self, ha_with_integration, reset_test_entities):
        """Switches should also be turned off."""
        group_name = "test_switch_target"

        await ha_with_integration.create_auto_off_group(
            group_name=group_name, sensors=["binary_sensor.test_motion"], targets=["switch.test_switch"], delay=0
        )
        await asyncio.sleep(2)
        ...
        try:
            ...
            await ha_with_integration.call_service(
                "input_boolean", "turn_on", {"entity_id": "input_boolean.test_switch_state"}
            )
            await asyncio.sleep(2)
...
class TestAutoOffWithDelay:
    ...
    async def test_delay_prevents_immediate_off(self, ha_with_integration, reset_test_entities):
    ...
    async def test_motion_during_delay_cancels_timer(self, ha_with_integration, reset_test_entities):
...
class TestEdgeCases:
    ...
    async def test_entity_unavailable(self, ha_with_integration, reset_test_entities):
    ...
    async def test_rapid_state_changes(self, ha_with_integration, reset_test_entities):
    ...
    async def test_update_group_config(self, ha_with_integration, reset_test_entities):
...
class TestTextEntities:
    ...
    async def test_delay_text_entity_created_for_group(self, ha_with_integration, reset_test_entities):
        """Test that delay text entity is created when a group is created."""
        group_name = "test_text_entities"

        await ha_with_integration.create_auto_off_group(
            group_name=group_name, sensors=["binary_sensor.test_motion"], targets=["light.test_light"], delay=60
        )
    ...
    async def test_edit_delay_via_text_entity(self, ha_with_integration, reset_test_entities):
        """Test editing delay via text entity."""
        group_name = "test_edit_delay"

        await ha_with_integration.create_auto_off_group(
            group_name=group_name, sensors=["binary_sensor.test_motion"], targets=["light.test_light"], delay=30
        )
        await asyncio.sleep(3)
        ...
        try:
            ...
            delay_state = await ha_with_integration.get_state(delay_entity)
            assert delay_state["state"] == "120", f"Delay text entity should show 120, got {delay_state['state']}"

        finally:
            await ha_with_integration.call_service("auto_off", "delete_group", {"group_name": group_name})

    async def test_edit_delay_with_template(self, ha_with_integration, reset_test_entities):
    ...
    async def test_delay_change_persists_in_config(self, ha_with_integration, reset_test_entities):
...
class TestDeadlineLogic:
    ...
    async def test_startup_sets_deadline_when_target_on_sensors_off(self, ha_with_integration, reset_test_entities):
        ...
        try:
            ...
            assert (
                ":" in deadline_state["state"]
            ), f"Deadline should be a time format HH:MM:SS, got: {deadline_state['state']}"

        finally:
            await ha_with_integration.call_service("auto_off", "delete_group", {"group_name": group_name})

    async def test_sensor_on_clears_deadline(self, ha_with_integration, reset_test_entities):
    ...
    async def test_sensors_off_sets_deadline(self, ha_with_integration, reset_test_entities):
    ...
    async def test_new_target_extends_deadline_only_if_later(self, ha_with_integration, reset_test_entities):
        ...
        try:
            ...
            assert second_time >= first_time, f"Deadline should be extended: {first_deadline} -> {second_deadline}"
